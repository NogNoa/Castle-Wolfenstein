...
fixit()
{
    ...
    if (SegmSt(0xb800, (byte*)1, 0xa5) != 0xa5) /*tryna to poke color screen buffer, is it there?*/
    {   setVideoMode(MDA);
        put_2_strings("Color graphics card not installed\n\r", "Program cannot execute");
        _exit(-1);
    }
    SegmSt(0xb800, (byte*) 1, 0);
    SegMemSet(SingleStep+1, 0x34);
    SegMemSet(SingleStep+3, 0xff);
    SegmSt(0x40, (byte*)0x10, SegmGt(0x40, (byte*) 0x10) & 0xcf | 0x10);
